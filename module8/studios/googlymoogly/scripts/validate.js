...
function validate(event) {
    ...
    var array = ['p','o','s','u','i','n','s'];
    var searchIsValid = search.length > 0;
    if (searchIsValid == false) {
        giveFeedback(searchContainer, "You must tell me your search!");
    }
    else if (invalidChars(search, array).length > 0){
        searchIsValid = false;
        giveFeedback(searchContainer, "The following characters are not allowed: p, o, c, u, i, n, s");
    }
...
function invalidChars(str, availableChars) {
    ...
    //      array.push()
    for (var item in str) { 
        if (availableChars.indexOf(availableChars[item]) > 0)
        invalids.push(str[item]);
    }
