...
int main(int argc, char* argv[])
{
	...
	while (true)
	{
		...
		if (connected())
		{
			...
			char* query = strtok(NULL, " "); // spliced again later
			char* version = strtok(NULL, "\r\n");
			char* abspath = strtok(query, "?");
			query = strtok(NULL, "\0");
			if(!query)
			{
				query = strchr(abspath, '\0');
			}
			char* extension = strchr(abspath, '.') + 1;
			
			// check http request formats
			if(!method || !abspath || !query || !version)
			{
				error(400);
				continue;
			}
			if(strchr(abspath, '"') || strchr(query, '"'))
			{
				error(400);
				continue;
			}
			...
			char path[strlen(root) + strlen(abspath) + 5];
			strncpy(path, root, strlen(root));
			path[strlen(root)] = '\0';
			strcat(path, abspath);
			
			// ensure path exists & is readable
...
const char* lookup(const char* extension)
{
	...
	char ext[strlen(extension) + 1];
	for(int i = 0, extlen = strlen(extension); i < extlen; i++)
		ext[i] = tolower(extension[i]);
	ext[strlen(extension)] = '\0';
	
	// check extension & return MIME type
	if(strcmp(ext, "css") == 0)
		return "text/css";
